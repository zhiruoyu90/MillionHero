...
/**
 * @author wangyunliang
 * @date 2018/1/12 0012 17:34
 */
public class BaiduAipMain {
	...
	private static void run() {
		//获取图片
		String imagePath = new Phone().getImagePath();
		//图像识别
		String questionAndAnswers;
		try {
			questionAndAnswers = BaiduAipUtils.recognizeRegion(imagePath);
		} catch (IOException e) {
			e.printStackTrace();
		...
		if (questionAndAnswers == null || !questionAndAnswers.contains(Constant.QUESTION_FLAG)) {
		...
		FutureTask futureQuestion = new FutureTask<Long>(new SearchAndOpen(question + " " + Constant.BAIDUBAIKE));
